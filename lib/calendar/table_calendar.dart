...
import '../AllPages/ExplorePages/Controllers/profile_controller.dart';
...
class _TableCalendarState<T> extends State<TableCalendar<T>> {
  ...
  void _onDayTapped(DateTime day) {
    ...
    var _data = cont
        .checkSlot(
            date: day.day, year: day.year, month: day.month, fullDate: day)
    ...
    if (_isDayDisabled(day) || _isFullBlock(_data) || _isBooked(_data)) {
  ...
  Widget _buildCell(DateTime day, DateTime focusedDay) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        Widget content = getCellContent(day, focusedDay, _data);
  ...
  _isPartialBlock(data) {
    if (data.isNotEmpty) {
      if (data.first.partiallyBlock) {
        return true;
      } else {
        return false;
      }
  ...
  _isBooked(data) {
    if (data.isNotEmpty) {
      if (data.first.completeDaybooked) {
        return true;
      } else {
        return false;
      }
  ...
  Widget getCellContent(DateTime day, DateTime focusedDay, dynamic data) {
    final isToday = isSameDay(day, widget.currentDay);
    final isDisabled = _isDayDisabled(day);
    final isWeekend = _isWeekend(day, weekendDays: widget.weekendDays);
    final isOutside = day.month != focusedDay.month;
    ...
    final isRangeStart = isSameDay(day, widget.rangeStartDay);
    final isRangeEnd = isSameDay(day, widget.rangeEndDay);
    final isFullBlock = _isFullBlock(data);
    final isPartialBlock = _isPartialBlock(data);
    final isBooked = _isBooked(data);
    return CellContent(
      block: widget.block,
      day: day,
      isFullBlock: isFullBlock,
      isPartiallyBlock: isPartialBlock,
      isBooked: isBooked,
      ...
      isOutside: isOutside,
      isDisabled: data.isEmpty ? true : isDisabled,
      isWeekend: isWeekend,
