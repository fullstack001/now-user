...
class CountryDialog extends StatefulWidget {
  ...
  CountryDialog({Key? key, this.countriesList}) : super(key: key);
                      ...
                      child: TextFormField(
                        ...
                        onChanged: (value) {
                          ...
                          if (value.isEmpty) {
                            ...
                            for (int index = 0;
                            index < widget.countriesList.length;
                            index++) {
                              if (widget.countriesList[index].name
                                  .toString()
                                  .toLowerCase()
                                  .contains(value)) {
                                list.add(widget.countriesList[index]);
                              }
                    ...
                    Expanded(
                        child: ListView.builder(
                          itemCount: filterList.isEmpty
                              ? widget.countriesList.length
                              : filterList.length,
                          itemBuilder: (BuildContext context, int index) {
                            CountryData value = filterList.isEmpty
                                ? widget.countriesList[index]
                                : filterList[index];
                            int size = filterList.isEmpty
                                ? widget.countriesList.length - 1
                                : filterList.length - 1;

                            return InkWell(
                              onTap: () {
                                Get.focusScope!.unfocus();
                                for (int index = 0; index < widget.countriesList.length; index++) {
                                  if (widget.countriesList[index].id == value.id) {
                                    _controller.selectedId.value =
                                        int.parse((value.id).toString());
                                    _controller.selectedCountryIndex.value = value.id!;

                                    if (_controller
                                        .countryList.value[index].cities.isNotEmpty) {
                                      int idd = _controller
                                          .countryList.value[index].cities.first.id;
                                      _controller.cityId(idd);
                                    }
                                    _controller.selectedCountryIndex.refresh();
                                    Get.back();
                                    return;
                                  }
                                }
